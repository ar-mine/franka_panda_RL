...
parser.add_argument('--env', type=str, default='PandaReachJointsDense-v2', help='PandaReachDense')
...
parser.add_argument('--action_limit', type=int, default=1, help='action limit')
parser.add_argument('--hidden_sizes', type=int, default=128, help='hidden_sizes')   # 32
parser.add_argument('--mode', type=str, default='test', help='train or test')
parser.add_argument('--test_model', default=1000, help='test model')
...
state_dim = int(env.observation_space['observation'].shape[0]/2)
...
class Agent():
    ...
    def load_model(self, model_name):
        name = "./model/policy%d" % model_name
        self.actor = torch.load("{}.pkl".format(name), map_location=device)
...
def main():
    ...
    if args.mode == 'train':
        ...
        print("#The model:", args.test_model, 'is loaded!!!')
    ...
    for episode in range(1, args.training_eps + 1):
        ...
        while not (done or step_number == args.max_step):
            ...
            if args.mode == 'train':
                ...
                action, _, _ = agent.select_action(relative_dis)
                next_obs, reward, done, _ = agent.env.step(action)
                next_relative_dis = next_obs['desired_goal'] - next_obs['observation'][0:3]

                # Add experience to replay buffer
                agent.replay_buffer.add(relative_dis, action, reward, next_relative_dis, done)
                ...
                relative_dis = obs['desired_goal'] - obs['observation'][0:3]
                mu, _ = agent.actor(torch.FloatTensor(relative_dis).to(device))
                action = action_limit*torch.tanh(mu).detach().cpu().numpy()
                next_obs, reward, done, _ = agent.env.step(action)
