...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("franka_control.demo_hand.command_executor");
...
//            gripper_.grasp(0.18, 0.04, 30);
            ...
            step_ = 0;
            transform_get(tf_buffer_, LOGGER, "panda_link0", "lattice_"+std::to_string(i), pose);
            pose.position.z = 0.4;
            move_group_.setPoseTarget(pose, "panda_hand_tcp");
            move_group_.move();
            pose.position.z = 0.3;
